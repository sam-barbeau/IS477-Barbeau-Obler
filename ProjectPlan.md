...
- **Other economic indicators:** There are several related FRED series (housing starts, construction spending, unemployment rate) that could add useful context. We may pull in one or two of these if time allows, but for now we're keeping the scope focused on the core rate-to-permits question.
